...
class Gateway_Stripe(PaymentGatewayManager):
    ...
    def process_webhook_response(self):
        ...
        if self.event['type'] == EVENT_CHECKOUT_SESSION_COMPLETED:
            ...
            # sleep for 4 seconds to ensure enough time for the other racing webhook to come in
            print("Sleep started - checkout.session.completed")
            time.sleep(4)
            self.donation.save()
            print("Saved donation - checkout.session.completed")
        ...
        if self.event['type'] == EVENT_PAYMENT_INTENT_SUCCEEDED:
            ...
            # sleep for 3 seconds to ensure enough time for the other racing webhook to come in
            # this should also be 1 second quicker than the checkout.session.completed
            # such that the new transaction_id is overridden by the old uuid generated by newstream
            print("Sleep started - payment_intent.succeeded")
            time.sleep(3)
            self.donation.save()
            print("Saved donation - payment_intent.succeeded")
