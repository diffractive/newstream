...
def global_admin_js():
    return mark_safe(
        '''<script>
        // custom js for correctly highlighting the nested tabs
        // todo: it would be a nice bonus if clicking these tabs save to navigation history, so user can click back to go back to last tab
        (function ($) {
            // on document ready
            $(function() {
                // run below only on Site Settings page
                if ($("body").hasClass("menu-settings")) {
            ...
            $(document).on("click", ".w-tabs__tab", (e) => {
                ...
                // open first child tab by default
