...
class DonationWebForm(forms.Form):
    ...
    opt_in_mailing_list = forms.BooleanField(label='Opt in Mailing List?', required=False)
    is_recurring = forms.BooleanField(widget=forms.HiddenInput(), required=False)

    def __init__(self, *args, blueprint=None, **kwargs):
        super().__init__(*args, **kwargs)
        if blueprint == None:
            raise django.core.exceptions.ObjectDoesNotExist('Please provide a DonationForm blueprint')
        form = blueprint
        ...
        if form.is_recurring:
            self.fields["is_create_account"] = forms.BooleanField(widget=forms.HiddenInput(), label='Create Account?', required=False, initial=form.is_recurring)
        else:
            self.fields["is_create_account"] = forms.BooleanField(label='Create Account?', required=False, initial=form.is_recurring)

        # construct payment gateway field
        gateways = form.allowed_gateways.all()
        self.fields["payment_gateway"] = forms.ChoiceField(choices=[(x.id, x.title) for x in gateways])
        ...
        if form.isAmountFixed():
            self.fields["donation_amount"] = forms.DecimalField(initial=form.fixed_amount)
            self.fields["donation_amount"].widget.attrs['readonly'] = True
        elif form.isAmountStepped():
            amountSteps = form.amount_steps.all()
            self.fields["donation_amount"] = forms.ChoiceField(choices=[(x.step,x.step) for x in amountSteps])
