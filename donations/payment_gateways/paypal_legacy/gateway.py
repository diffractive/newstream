...
from donations.email_functions import sendRenewalReceiptToDonor, sendRenewalNotifToAdmins, sendRecurringCancelRequestNotifToAdmins, sendRecurringCancelledNotifToAdmins, sendRecurringCancelledNotifToDonor
...
class Gateway_Paypal_Legacy(PaymentGatewayManager):
    ...
    def process_webhook_response(self):
        if self.request.POST.get('txn_type', None) == 'subscr_payment':
            ...
            _debug("[paypal legacy recurring] ipn txn_type not handled: "+self.request.POST.get('txn_type', None))
